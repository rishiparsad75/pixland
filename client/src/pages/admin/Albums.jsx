...
import { Image as ImageIcon, MapPin, Calendar, Camera, Globe, Search } from "lucide-react";
...
const EventMonitor = () => {
    ...
    useEffect(() => {
        const fetchAllEvents = async () => {
            try {
                // In a full SaaS admin, we'd have a SuperAdmin GET /api/events/all
                // For now, we'll fetch my-events if the SuperAdmin is also a photographer, 
                // or assume an endpoint exists for the system view.
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-white">
                    {filteredEvents.map((event) => (
                        <Card key={event._id} className="group hover:border-indigo-500/50 transition-all duration-300 bg-white/5 backdrop-blur-xl border-white/10 p-0 overflow-hidden flex flex-col h-full">
                            <div className="h-40 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 relative flex items-center justify-center">
                                ...
                                <div className="absolute top-4 right-4 px-2 py-1 bg-black/60 backdrop-blur-md rounded-lg border border-white/10 text-[10px] font-bold uppercase tracking-widest text-indigo-400">
                                    Active Project
                                </div>
                            </div>
